...
pub async fn push_lead_to_adaswift(
    ...
    Ok(Json(
        json!({"status":"pushed","app":"ADASwift","email":email}),
    ))
}
...
pub async fn provision_adaswift_user(
    ...
    Ok(Json(
        json!({"status":"provisioned","app":"ADASwift","email":email,"plan":plan,"affiliate_id":affiliate_id,"message":format!("User '{}' provisioned in ADASwift ({})",name,plan)}),
    ))
}
...
pub async fn adaswift_health(
    ...
    Ok(Json(
        json!({"connected":true,"url":"https://adaswift.com","status":"healthy"}),
    ))
}
