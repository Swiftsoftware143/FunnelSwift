...
pub async fn list_legal_pages(State(state): State<AppState>) -> AppResult<Json<Value>> {
    let rows = sqlx::query(
...
pub async fn save_legal_page(
    ...
    ).bind(&payload.slug).fetch_optional(&state.pool).await?;
...
pub async fn delete_legal_page(
    ...
    let result = sqlx::query("DELETE FROM zaarhub_legal_pages WHERE slug = $1")
        .bind(&slug).execute(&state.pool).await?;
