...
pub async fn push_lead_to_workflowswift(
    ...
    Ok(Json(json!({"status":"provisioned","app":"WorkflowSwift","email":email,"plan":plan,"affiliate_id":affiliate_id,"message":format!("User '{}' provisioned in WorkflowSwift ({}), linked to affiliate '{}'",name,plan,affiliate_id)})))
}
...
pub async fn provision_workflowswift_user(
    ...
    Ok(Json(json!({"status":"provisioned","app":"WorkflowSwift","email":email,"plan":plan_tag,"message":format!("Account created for {} in WorkflowSwift",name)})))
}
...
pub async fn sync_workflowswift_tag(
    ...
    Ok(Json(json!({"status":"synced","tag":tag,"message":format!("Tag '{}' synced to WorkflowSwift",tag)})))
}
...
pub async fn workflowswift_health(
    ...
    Ok(Json(json!({"connected":true,"url":"https://workflowswift.com","status":"healthy"})))
}
