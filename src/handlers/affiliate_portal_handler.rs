...
pub async fn affiliate_signup(
    ...
    let email = payload["email"]
        .as_str()
        .ok_or_else(|| AppError::Validation("Email required".into()))?;
...
pub async fn affiliate_login(
    ...
    let row: (String, String, String, bool) =
        sqlx::query_as("SELECT id, name, email, is_active FROM affiliates WHERE email = $1")
            .bind(email)
    ...
    Ok((
        StatusCode::OK,
        Json(json!({"id": row.0, "name": row.1, "email": row.2, "is_active": row.3})),
    ))
}
...
pub async fn affiliate_portal_dashboard(
    ...
    Ok(Json(
        json!({"total_leads": row.0, "total_earnings": row.1.unwrap_or(0.0)}),
    ))
}
