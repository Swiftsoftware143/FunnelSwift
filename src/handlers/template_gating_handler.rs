...
pub async fn update_plan_templates(
    ...
    let template_ids: Option<Vec<String>> = match req.get("allowed_template_ids") {
        ...
        Some(v) => v.as_array().map(|arr| arr.iter().filter_map(|x| x.as_str().map(String::from)).collect()),
        None => return Err(AppError::BadRequest("allowed_template_ids field required".into())),
    };

    sqlx::query("UPDATE plans SET allowed_template_ids = $1 WHERE id = $2")
        .bind(&template_ids).bind(id).execute(&state.pool).await?;
