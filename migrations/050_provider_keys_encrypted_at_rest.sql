-- 050_provider_keys_encrypted_at_rest.sql
--
-- provider_keys.api_key holds CUSTOMER-SUPPLIED third-party credentials (OpenAI, Resend /
-- SendGrid, Mailgun, CoreSwift hub keys). Before this migration the write path bound the raw
-- request value straight into the column, so every BYOK key a tenant entered sat in the
...
--
-- The no-semicolon rule applies to every comment above: the migration runner (and the
-- deploy staging path) splits files on the statement separator, so a comment must never
-- contain one.
