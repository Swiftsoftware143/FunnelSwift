...
/* ═══ PLAN GATING ═══ */
var PU = null; // plan usage cache
function loadUsage() {
  return api("/api/v1/me/usage").then(function(d) { PU = d; return d; }).catch(function() { PU = null; });
}
function planLimit(key) {
  // Get limit from current_plan stored in U
  if (!U || !U.current_plan) return null;
  var p = U.current_plan;
  var map = {
    cards: p.max_cards, kinetic_cards: p.max_cards, max_cards: p.max_cards,
    leads: p.max_leads, max_leads: p.max_leads,
    tags: p.max_tags, max_tags: p.max_tags,
    forms: p.max_forms, max_forms: p.max_forms,
    domains: p.max_custom_domains, max_custom_domains: p.max_custom_domains, max_domains: p.max_custom_domains,
    team: p.max_team_members, max_team_members: p.max_team_members, team_members: p.max_team_members
  };
  return map[key] !== undefined ? map[key] : null;
}
function planUsage(key) {
  if (!PU) return 0;
  var map = {cards:"cards",max_cards:"cards",leads:"leads",max_leads:"leads",tags:"tags",max_tags:"tags",forms:"forms",domains:"domains",team:"team"};
  var k = map[key] || key;
  return PU[k] || 0;
}
function checkPlanLimit(key, label, callback) {
  // Check before an action. If at limit, show upgrade prompt. Otherwise call callback().
  var limit = planLimit(key);
  if (limit === null || limit === undefined) { callback(); return; } // no limit set
  if (limit === 0) { showUpgradePrompt(label + " is not available on your current plan. Please upgrade to access this feature."); return; }
  loadUsage().then(function() {
    var usage = planUsage(key);
    if (usage >= limit) {
      showUpgradePrompt(label + " limit reached (" + usage + "/" + limit + "). Upgrade to increase your limit.");
    } else {
      callback();
    }
  });
...
function showUpgradePrompt(msg) {
  if (!msg) msg = "You have reached your plan limit. Upgrade to unlock more.";
  O("Upgrade Required", function() {
    $("mb").innerHTML = '<div style="text-align:center;padding:20px"><p style="font-size:15px;color:#1e1b4b;margin-bottom:16px">' + msg + '</p><button class="btn" onclick="C();S("upgrade")" style="padding:12px 24px;font-size:14px">View Plans &amp; Upgrade</button></div>';
  });
}
/* Show upgrade prompt on 402 Payment Required */
var _origApi = api;
api = function(p, o) {
  ...
  return fetch(p, o).then(function(r) {
    if (r.status === 402) {
      return r.json().then(function(e) {
        showUpgradePrompt(e.error || "Plan limit reached. Upgrade to continue.");
